...
DB_URL = f"postgresql+psycopg2://{os.getenv('POSTGRES_USER')}:{os.getenv('POSTGRES_PASSWORD')}@postgres_db:5432/{os.getenv('POSTGRES_DB')}"
...
def delete_account(db: Session, account_id: int) -> bool:
    ...
    # Deleta operações associadas primeiro (se cascade nao estiver configurado)
    db.query(Operations).filter(Operations.account_id == account_id).delete()
